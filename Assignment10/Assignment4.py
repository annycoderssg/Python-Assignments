...
def scanDirectory( sourcePath, destinationPath, newExtension ):
    ...
    if os.path.isdir(sourcePath):
        for dirName, subDirs, fileList in os.walk( sourcePath ):
            for file in fileList:
                ...
                pre, ext = os.path.splitext(dst_file)
                os.rename(dst_file, pre+newExtension)
                print("file Copied & Change extension : ", pre+newExtension )     
...
def main():
    ...
    if( argv[1] == "-h" or argv[1] == "-H" ):       # Flag for displaying usage of help
        ...
        print( "Usage: Application_name Folder_1 Folder_2 NewExtension")
        print( "Example: Assignment4.py 'Demo' 'Copy' '.exe")
        exit()
